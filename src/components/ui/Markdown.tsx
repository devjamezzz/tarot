  ...
  return (
    <div className="space-y-3">
      {nodes.map((n, idx) => {
        if (n.type === "h4") {
          return (
            <h4 key={idx} className="font-display text-base font-semibold text-gold">
              {n.text}
            </h4>
          );
        }
        ...
        if (n.type === "ul") {
          return (
            <ul key={idx} className="list-disc space-y-1 pl-5 text-sm leading-relaxed text-fg-muted marker:text-gold">
              {n.items.map((it, i) => (
                <li key={i}>{it}</li>
              ))}
            </ul>
